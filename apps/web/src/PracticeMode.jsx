...
// ── Fireworks (shown on self-draw wins) ──────────────────────────────────────
const SPARK_COLORS = ['#ffd700', '#ff6b6b', '#ff9f43', '#ffffff', '#ff4757', '#ffeaa7', '#fdcb6e', '#74b9ff']
const BURST_POSITIONS = [
  { x: 18, y: 12, baseDelay: 0    },
  { x: 82, y: 12, baseDelay: 0.5  },
  { x: 50, y: 6,  baseDelay: 1.0  },
  { x: 12, y: 50, baseDelay: 1.5  },
  { x: 88, y: 50, baseDelay: 0.25 },
  { x: 30, y: 80, baseDelay: 0.75 },
  { x: 70, y: 80, baseDelay: 1.25 },
]
const ANGLES = [0, 45, 90, 135, 180, 225, 270, 315, 22.5, 67.5, 112.5, 157.5]

function Fireworks() {
  return (
    <div className="absolute inset-0 pointer-events-none overflow-hidden z-10">
      {BURST_POSITIONS.map((burst, bi) =>
        ANGLES.map((angle, ai) => (
          <span
            key={`${bi}-${ai}`}
            ...
            style={{
              ...
              '--a':     `${angle}deg`,
              '--d':     `${48 + (ai % 4) * 12}px`,
              '--color': SPARK_COLORS[(bi + ai) % SPARK_COLORS.length],
              '--delay': `${burst.baseDelay + ai * 0.025}s`,
            }}
          />
        ))
      )}
...
export default function PracticeMode({ onExit }) {
  ...
  const { balance, winMoney } = useMoneyStore()
  ...
  const triggerWin = (type, hand, exposed, flowers) => {
    setWinType(type)
    const runScore = calculateScore({ hand, exposed, flowers, winType: type })
    setWinScore(runScore)
    setGamePhase('win')
    
    // Add Point System Winnings
    const payout = 5 + (runScore.total * 1)
    winMoney(payout)
  }
  ...
  return (
    <div className={isMobile ? "absolute inset-0 w-full h-full flex flex-col overflow-hidden bg-felt text-white" : "grid grid-cols-1 md:grid-cols-3 grid-rows-[1fr_auto_auto] md:grid-rows-[1fr_auto] h-screen w-screen overflow-hidden bg-felt text-white relative"}>
         ...
         <div className="absolute inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-md">
            ...
            <div className="bg-red-700 border-4 border-yellow-400 p-8 sm:p-12 rounded-3xl shadow-[0_0_150px_rgba(255,0,0,1)] transform text-center flex flex-col items-center animate-in fade-in zoom-in duration-500">
               ...
               <h2 className="text-2xl sm:text-3xl font-bold text-yellow-100 drop-shadow-md tracking-wider">
                 {winType === 'zimo' ? 'Self-Drawn Victory' : 'Winner!'}
               </h2>
               ...
               {winScore && (
                 <div className="w-full border-t border-yellow-400/30 pt-4 mt-4 mb-6">
                   {winScore.breakdown.length > 0 ? (
                     ...
                     <p className="text-yellow-100/50 text-sm text-center italic mb-3">No bonus points this round.</p>
                   )}
                   <div className="flex justify-between items-center border-t border-yellow-400/40 pt-2 px-2">
                     ...
                     <span className="font-black text-2xl text-yellow-300">{winScore.total} tai</span>
                   </div>
                 </div>
               )}
               
               {winScore && (
                  <div className="bg-black/40 p-4 rounded-xl w-full mb-6 border border-yellow-500/30 shadow-inner">
                     <p className="text-emerald-400 font-bold mb-1 uppercase tracking-widest text-sm">Payout Collected</p>
                     <p className="text-white text-3xl font-black text-emerald-300">
                        +${5 + (winScore.total * 1)}
                     </p>
                  </div>
               )}
      ...
      <div className={isMobile ? "flex-1 relative z-0 flex flex-col min-h-0 shadow-[inset_0_0_100px_rgba(0,0,0,0.5)]" : "col-start-1 row-start-1 md:col-start-1 md:col-span-2 md:row-start-1 relative z-0 shadow-[inset_0_0_100px_rgba(0,0,0,0.5)] flex flex-col min-h-0"}>
         ...
         <div className={`absolute top-4 left-4 z-10 flex flex-col sm:flex-row items-start sm:items-center gap-2 sm:gap-4`}>
             <div className={`px-3 sm:px-4 py-1.5 sm:py-2 text-xs sm:text-base font-mono bg-black/40 rounded border border-white/20 text-white/90 shadow-xl transition-colors duration-500 flex gap-2 items-center tracking-wider ${currentTurn === 0 ? 'border-emerald-500 text-emerald-300 shadow-[0_0_20px_rgba(16,185,129,0.3)] backdrop-blur-sm' : 'border-white/10 backdrop-blur-sm'}`}>
                <span className={`w-2 h-2 sm:w-3 sm:h-3 rounded-full ${currentTurn === 0 ? 'bg-emerald-400 animate-pulse outline outline-emerald-400/50 outline-2 outline-offset-1' : 'bg-white/20'}`}></span>
                Remaining: {remainingDeck.length}
             </div>
             <div className="px-3 sm:px-4 py-1.5 sm:py-2 text-xs sm:text-base font-mono bg-emerald-900/60 rounded border border-emerald-500/50 text-emerald-300 shadow-[0_0_15px_rgba(16,185,129,0.2)]">
                Bank: ${balance}
             </div>
         </div>
         
         <div className="absolute top-4 right-4 z-10 flex flex-col gap-2">
            <button onClick={onExit} className="px-3 sm:px-4 py-1.5 sm:py-2 bg-red-900/80 border border-red-600 rounded text-xs sm:text-base shadow-sm hover:bg-red-800 transition w-max text-white">Exit</button>
